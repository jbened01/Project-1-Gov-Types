...
```{r}
library(tidyverse)
my_function <- function(lgl_var, spatial_democracy = democracy_data$spatial_democracy){
  valid <- !is.na(lgl_var)
...
vars <- c("is_monarchy", "is_commonwealth", "is_female_monarch", "is_democracy", "is_presidential", "is_interim_phase", "is_female_president", "is_colony", "is_communist", "has_proportional_voting", "has_full_suffrage", "is_multiparty", "has_free_and_fair_election")

map(vars, function(var) {
  my_function(democracy_data[[var]])
})
...
result_df <- map_dfr(vars, function(var) {
  res <- my_function(democracy_data[[var]])
