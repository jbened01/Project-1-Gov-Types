...
::: {.card title = "Income per Capita by State in 2025"}
...
combined <- meta_data |>
  ...
  group_by(regime_category) |>
  mutate(regime_group = if_else(n() < 100, "Territories & Colonies", first(regime_category))) |>
  ungroup()

dem_gdp <- combined |>
  mutate(democracy = if_else(is_democracy == 1, "Democracy", "Non-Democracy")) |>
  group_by(year, democracy) |>
  summarize(mean_gdp = mean(gdp_per_capita_nominal, 
                            na.rm = TRUE), 
                            .groups = "drop")
